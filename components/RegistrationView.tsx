    ...
    return (
      <div className="flex-1 flex flex-col h-full bg-background overflow-hidden animate-in fade-in duration-700">
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          <div className="max-w-4xl mx-auto w-full px-6 py-12 md:py-24 space-y-12 md:space-y-16">
            <div className="text-center space-y-4">
              ...
              <p className="text-sm md:text-lg text-zinc-500 max-w-xl mx-auto font-light leading-relaxed">Choose your entry point into the Base ecosystem. Create a new entity from scratch or bridge your existing architecture.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              <button 
                onClick={() => startRegistration(true)}
                className="group p-6 md:p-10 rounded-[2rem] bg-surface/20 border border-border hover:border-primary/50 transition-all duration-500 text-left space-y-4 relative overflow-hidden"
              >
                <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity">
                  <Plus className="w-16 h-16" />
                </div>
                <div className="w-12 h-12 md:w-16 md:h-16 rounded-2xl bg-primary flex items-center justify-center text-black shadow-xl group-hover:scale-110 transition-transform">
                  <Sparkles className="w-6 h-6 md:w-8 md:h-8" />
                </div>
                <div className="space-y-2">
                  <h3 className="text-xl md:text-2xl font-bold text-primary">Incept New Business</h3>
                  <p className="text-xs md:text-sm text-zinc-500 font-light leading-relaxed">Synthesize a fresh venture architecture, deploy new smart contracts, and launch on Base.</p>
                </div>
                <div className="flex items-center gap-2 text-[10px] font-bold text-primary uppercase tracking-[0.2em] pt-2">
              ...
              <button 
                ...
                className="group p-6 md:p-10 rounded-[2rem] bg-surface/20 border border-border hover:border-emerald-500/50 transition-all duration-500 text-left space-y-4 relative overflow-hidden"
              >
                <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity">
                  <History className="w-16 h-16" />
                </div>
                <div className="w-12 h-12 md:w-16 md:h-16 rounded-2xl bg-zinc-900 border border-border flex items-center justify-center text-emerald-500 group-hover:scale-110 transition-transform">
                  <Plus className="w-6 h-6 md:w-8 md:h-8" />
                </div>
                <div className="space-y-2">
                  <h3 className="text-xl md:text-2xl font-bold text-primary">Plug-in Existing Asset</h3>
                  <p className="text-xs md:text-sm text-zinc-500 font-light leading-relaxed">Import an already existing business protocol or contract to leverage the Venyro yield engine.</p>
                </div>
                <div className="flex items-center gap-2 text-[10px] font-bold text-emerald-500 uppercase tracking-[0.2em] pt-2">
            ...
            <div className="text-center pt-8">
               <button 
                onClick={() => onExit()}
                className="text-[10px] font-bold text-zinc-600 hover:text-zinc-400 transition-colors uppercase tracking-[0.3em] flex items-center gap-3 mx-auto"
               >
