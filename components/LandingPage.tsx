  ...
  return (
    <div className="w-full h-screen bg-background relative flex flex-col overflow-x-hidden">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden select-none">
        ...
        <div className="absolute top-[-10%] left-[-10%] w-[120%] h-[70%] bg-white/[0.02] rounded-full blur-[150px] animate-float"></div>
        <div className="absolute bottom-[-10%] right-[-10%] w-[120%] h-[70%] bg-zinc-500/[0.03] rounded-full blur-[150px] animate-float" style={{ animationDelay: '3s' }}></div>
        <div className="absolute inset-0 bg-gradient-to-b from-background via-transparent to-background"></div>
      </div>

      {/* Static Header */}
      <nav className="h-20 md:h-24 border-b border-border/40 bg-background/50 backdrop-blur-3xl sticky top-0 z-50 flex items-center justify-between px-4 sm:px-8 lg:px-16 shrink-0 w-full overflow-hidden">
        ...
        <div className="flex items-center gap-3 md:gap-10">
          ...
          <div className="flex items-center gap-2 md:gap-6">
            ...
            <button 
              ...
              className="bg-primary text-background px-4 sm:px-7 py-2 sm:py-3 rounded-xl md:rounded-2xl font-medium text-[9px] sm:text-xs uppercase tracking-widest hover:bg-white transition-all shadow-2xl shadow-white/5 active:scale-95 transform flex items-center gap-2 whitespace-nowrap"
            >
      ...
      <div className="flex-1 overflow-y-auto relative z-10 scroll-smooth overflow-x-hidden">
        {/* Hero Section */}
        <section className="flex flex-col items-center justify-center px-4 pt-16 md:pt-32 pb-24 md:pb-48 max-w-7xl mx-auto text-center overflow-hidden">
          <div className="inline-flex items-center gap-3 px-4 md:px-6 py-2 rounded-full bg-zinc-900/50 border border-white/5 text-[8px] md:text-[10px] font-medium text-zinc-500 uppercase tracking-[0.3em] mb-8 md:mb-12 animate-in slide-in-from-bottom-2 duration-500 backdrop-blur-md whitespace-nowrap">
          ...
          <h1 className="text-3xl sm:text-5xl md:text-7xl lg:text-[9rem] font-semibold text-primary tracking-tighter leading-[1.1] md:leading-[0.9] mb-8 md:mb-12 animate-in fade-in slide-in-from-bottom-4 duration-1000 max-w-[98vw] sm:max-w-none px-2">
            Venture architecture,<br className="hidden sm:block" />
            <span className="text-zinc-600">at machine speed.</span>
          </h1>
          
          <p className="text-sm sm:text-lg lg:text-2xl text-zinc-500 max-w-2xl mx-auto mb-10 md:mb-16 leading-relaxed font-light animate-in fade-in slide-in-from-bottom-6 duration-1000 px-6 sm:px-4">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 md:gap-6 w-full sm:w-auto animate-in fade-in slide-in-from-bottom-8 duration-1000 px-6 sm:px-0">
            <button 
              onClick={onGetStarted}
              className="w-full sm:w-auto px-8 md:px-20 py-4 md:py-7 bg-primary text-background rounded-2xl md:rounded-[2.5rem] font-semibold text-xs md:text-base uppercase tracking-widest hover:bg-white transition-all shadow-2xl shadow-white/10 flex items-center justify-center gap-3 active:scale-95 transform group"
            >
              Start Synthesis <ArrowRight className="w-4 h-4 md:w-6 md:h-6 group-hover:translate-x-1 transition-transform" />
            </button>
            <button onClick={() => onNavigate('FRAMEWORK')} className="w-full sm:w-auto px-8 md:px-20 py-4 md:py-7 bg-surface/30 border border-border/80 rounded-2xl md:rounded-[2.5rem] font-medium text-xs md:text-base uppercase tracking-widest text-zinc-400 hover:text-primary hover:bg-surface/50 transition-all flex items-center justify-center gap-3 backdrop-blur-sm">
              Explore Framework
            </button>
          </div>
        </section>
        ...
        <section className="px-4 sm:px-12 py-24 md:py-56 border-t border-border/40 relative overflow-hidden">
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-32 items-center">
            <div className="space-y-10 md:space-y-12">
              <div className="space-y-4 md:space-y-6">
                ...
                <h2 className="text-3xl md:text-6xl font-medium text-primary tracking-tight">The Full Venture Lifecycle.</h2>
                <p className="text-base md:text-lg text-zinc-500 font-light leading-relaxed px-1">Venyro doesn't just stop at strategy. We provide the infrastructure to legalize, build, and trade your high-fidelity assets.</p>
              </div>

              <div className="space-y-8 md:space-y-10">
                <div className="flex gap-4 md:gap-6 group cursor-default">
                  <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-zinc-900 border border-border flex items-center justify-center text-zinc-500 group-hover:text-primary transition-all duration-500 shrink-0">
                    <ShieldCheck className="w-6 h-6 md:w-7 md:h-7" />
                  </div>
                  <div className="space-y-1 md:space-y-2">
                    <h3 className="text-lg md:text-xl font-medium text-primary">Incorporation Terminal</h3>
                ...
                <div className="flex gap-4 md:gap-6 group cursor-default">
                  ...
                  <div className="space-y-1 md:space-y-2">
                    <h3 className="text-lg md:text-xl font-medium text-primary">Strategic Marketplace</h3>
            ...
            <div className="relative group p-2 overflow-visible">
              <div className="aspect-square rounded-[2rem] md:rounded-[3rem] bg-gradient-to-tr from-zinc-900 to-zinc-950 border border-border p-6 md:p-12 overflow-hidden shadow-2xl relative z-10">
                ...
                <div className="relative z-10 flex flex-col justify-between h-full gap-4">
                  <div className="p-4 md:p-6 rounded-2xl md:rounded-[2rem] bg-surface/80 border border-white/5 backdrop-blur-xl shadow-2xl animate-in slide-in-from-top-4 duration-1000">
                    ...
                    <p className="text-xs md:text-sm font-medium text-primary">Venture Registration Status: 92% Complete</p>
                  ...
                  <div className="p-4 md:p-6 rounded-2xl md:rounded-[2rem] bg-primary text-background shadow-2xl self-end max-w-[85%] animate-in slide-in-from-bottom-4 duration-1000 delay-300">
                    <div className="flex items-center gap-2 md:gap-3 mb-1.5 md:mb-2">
                      ...
                      <span className="text-[8px] md:text-[9px] font-bold uppercase tracking-widest">New Listing</span>
                    </div>
                    <p className="text-xs md:text-base font-bold tracking-tight mb-0.5 md:mb-1">EcoSaaS Protocol</p>
                    <p className="text-[8px] md:text-[10px] font-medium opacity-70 uppercase tracking-wider">Current Yield: $8.2k/mo</p>
                  </div>
                </div>
              </div>
              {/* Decorative elements contained via -z-10 and relative parent */}
              ...
              <div className="absolute -top-6 -right-6 w-32 h-32 md:w-48 md:h-48 bg-emerald-500/5 rounded-full blur-[100px] animate-pulse -z-0" style={{ animationDelay: '2s' }}></div>
            </div>
          </div>
        </section>

        {/* Feature Grid Section */}
        <section id="features" className="px-6 md:px-8 py-24 md:py-56 bg-zinc-950/40 border-t border-border/40 backdrop-blur-sm overflow-hidden">
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-16 md:gap-24 lg:gap-36">
            <div className="space-y-6 group">
              <div className="w-16 h-16 rounded-2xl bg-zinc-900 border border-border flex items-center justify-center text-zinc-600 group-hover:text-primary group-hover:border-primary/40 transition-all group-hover:-translate-y-2 duration-500 shadow-2xl">
        ...
        {/* CTA Section */}
        <section className="px-6 md:px-8 py-24 md:py-56 border-t border-border/40 bg-gradient-to-b from-zinc-950/20 to-background text-center overflow-hidden">
          <div className="max-w-4xl mx-auto space-y-10 md:space-y-16">
            <h2 className="text-3xl sm:text-5xl md:text-7xl lg:text-8xl font-medium text-primary tracking-tight leading-tight px-4">Ready to synthesize?</h2>
            ...
            <button 
              onClick={onGetStarted}
              className="w-full sm:w-auto px-10 md:px-24 py-5 md:py-8 bg-primary text-background rounded-2xl md:rounded-[3rem] font-semibold text-sm md:text-lg uppercase tracking-widest hover:bg-white transition-all shadow-2xl active:scale-95 transform"
            >
        ...
        <footer className="border-t border-border/40 px-6 md:px-12 py-16 md:py-32 bg-background relative z-10 w-full overflow-hidden">
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-24 mb-16">
            ...
            <div className="space-y-6">
              ...
              <nav className="flex flex-col gap-3 md:gap-4">
                <button onClick={() => onNavigate('FRAMEWORK')} className="text-xs sm:text-sm md:text-base text-zinc-500 hover:text-primary transition-colors text-left font-light">Methodology</button>
                <button onClick={() => onNavigate('VENTURES')} className="text-xs sm:text-sm md:text-base text-zinc-500 hover:text-primary transition-colors text-left font-light">Showcase</button>
                <button onClick={() => onNavigate('PRICING')} className="text-xs sm:text-sm md:text-base text-zinc-500 hover:text-primary transition-colors text-left font-light">Pricing</button>
              </nav>
            </div>
            <div className="space-y-6">
              <h4 className="text-[10px] md:text-[12px] uppercase font-medium tracking-[0.4em] text-zinc-400">Institutional</h4>
              <nav className="flex flex-col gap-3 md:gap-4">
                <a href="#" className="text-xs sm:text-sm md:text-base text-zinc-500 hover:text-primary transition-colors font-light">Privacy Framework</a>
                <a href="#" className="text-xs sm:text-sm md:text-base text-zinc-500 hover:text-primary transition-colors font-light">Security Posture</a>
              </nav>
            </div>
          </div>
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6 border-t border-zinc-900 pt-10">
            <p className="text-[8px] md:text-[11px] text-zinc-700 uppercase tracking-[0.5em] font-medium text-center">Venyro Strategic Labs © 2025</p>
            <div className="flex gap-6 md:gap-12">
               <a href="#" className="text-[8px] md:text-[11px] text-zinc-600 hover:text-zinc-400 font-medium uppercase tracking-[0.3em] transition-colors">Twitter</a>
               <a href="#" className="text-[8px] md:text-[11px] text-zinc-600 hover:text-zinc-400 font-medium uppercase tracking-[0.3em] transition-colors">LinkedIn</a>
            </div>
          </div>
        </footer>
      </div>
