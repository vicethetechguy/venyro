...
const BusinessAcquisitionPage: React.FC<BusinessAcquisitionPageProps> = ({ 
  ...
  const handlePlaceBid = async () => {
    ...
    // Simulate smart contract escrow on Base
    ...
    <div className="fixed inset-0 z-[500] bg-background animate-in fade-in slide-in-from-right-4 duration-500 flex flex-col overflow-hidden">
      ...
      <nav className="h-16 md:h-20 border-b border-white/5 bg-zinc-950 px-4 md:px-8 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-4">
          <button onClick={() => setShowAuction(false)} className="p-2 hover:bg-white/5 rounded-full transition-colors">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-mono">Protocol: {inputs.productName}</p>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden bg-black">
        {/* Left Column: Market Depth & Order Book */}
        <div className="w-full lg:w-72 xl:w-80 border-r border-white/5 flex flex-col shrink-0 bg-zinc-950/50">
          <div className="p-4 border-b border-white/5 flex items-center justify-between">
            <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest flex items-center gap-2">
              <History className="w-3 h-3" /> Bid History
            </h3>
            <span className="text-[8px] font-bold text-zinc-700">LIVE</span>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar">
            {[
              { addr: '0x8f2...e421', amount: 245000, time: '2m ago' },
              { addr: '0x1a4...d390', amount: 238500, time: '14m ago' },
              { addr: '0x7c9...b112', amount: 230000, time: '1h ago' },
              { addr: '0x2d4...f901', amount: 215000, time: '4h ago' },
              { addr: '0xef0...a842', amount: 198000, time: 'Yesterday' },
            ].map((bid, i) => (
        ...
        {/* Middle Column: Visualizations & Data */}
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="p-4 md:p-8 flex-1 overflow-y-auto space-y-8 no-scrollbar">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-6 rounded-2xl bg-zinc-900 border border-white/5 space-y-2">
                   ...
                   <h4 className="text-3xl font-bold text-primary font-mono">${(highestBid * 4.5).toLocaleString()}</h4>
                ...
                <div className="p-6 rounded-2xl bg-zinc-900 border border-white/5 space-y-2">
                   ...
                   <h4 className="text-3xl font-bold text-primary font-mono">FIN-094</h4>
                   <p className="text-[9px] text-zinc-600">Current bidding window epoch.</p>
                </div>
             </div>

             <div className="p-6 md:p-10 rounded-[2.5rem] bg-zinc-950 border border-white/5 space-y-6">
                <div className="flex items-center justify-between">
        ...
        <div className="w-full lg:w-80 xl:w-96 border-l border-white/5 flex flex-col shrink-0 bg-zinc-950">
          ...
          <div className="flex-1 p-6 space-y-8">
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                 <label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest ml-1">Place Your Bid (USDC)</label>
                 <div className="relative">
                    <input 
                      ...
                      className="w-full bg-zinc-900 border border-white/10 rounded-xl px-5 py-4 text-xl font-bold font-mono text-primary focus:outline-none focus:border-primary/40 transition-all placeholder:text-zinc-800"
                    />
              ...
              {bidSuccess ? (
                <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center gap-3 animate-in zoom-in duration-300">
                  <CheckCircle2 className="w-5 h-5 text-emerald-500" />
                  <p className="text-[10px] font-bold text-emerald-500 uppercase">Bid Placed Successfully</p>
                </div>
              ) : (
                <button 
                  onClick={handlePlaceBid}
                  disabled={isPlacingBid || !bidAmount || parseFloat(bidAmount) <= highestBid}
                  className="w-full py-5 bg-primary text-black rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-white active:scale-95 transition-all disabled:opacity-30 shadow-xl flex items-center justify-center gap-3"
                >
                  {isPlacingBid ? <Loader2 className="w-5 h-5 animate-spin" /> : <><Zap className="w-4 h-4 fill-current" /> Confirm Bid</>}
                </button>
              )}
            </div>
            ...
            <div className="space-y-4">
               <h4 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest flex items-center gap-2">
                 <Info className="w-3 h-3" /> Protocol Rules
               </h4>
               <ul className="space-y-3">
                 {[
                   'Bids are escrowed in a smart contract.',
          ...
          <div className="p-6 bg-zinc-900/50 border-t border-white/5 space-y-4">
             ...
             <button className="w-full py-3 border border-white/10 rounded-xl text-[9px] font-bold text-zinc-400 uppercase tracking-widest hover:text-primary transition-all">
                Add Funds
             </button>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-[#050505] text-white selection:bg-primary/20 font-sans relative overflow-x-hidden pb-20">
           ...
           <div className="animate-in fade-in duration-500">
              {activeTab === 'APPRAISAL' && (
                <div className="space-y-6">
                  <div className="h-40 md:h-48 bg-zinc-800/20 rounded-2xl border border-white/5 flex items-center justify-center italic text-zinc-600 text-xs text-center px-4">
                    On-chain Revenue Layer: Syncing Base Settlement Logs...
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     <div className="space-y-2">
              ...
              {activeTab === 'STAKE' && (
                <div className="space-y-8 max-w-2xl mx-auto py-4">
                        ...
                        <div className="space-y-4">
                           <div className="space-y-2">
                              <div className="flex justify-between items-center px-1">
                                ...
                                <span className="text-[9px] text-zinc-600 font-mono">Balance: 4,281.50</span>
                              </div>
                              <div className="relative">
                                 <input 
                                  type="number" 
                                  ...
                                  onChange={(e) => setBidAmount(e.target.value)}
                                  placeholder="0.00"
                                  className="w-full bg-white/5 border border-white/10 rounded-xl px-5 py-4 text-xl font-bold text-primary focus:outline-none focus:border-primary/40 transition-all placeholder:text-zinc-800"
                                 />
                                 <button onClick={() => setBidAmount('4281')} className="absolute right-4 top-1/2 -translate-y-1/2 px-2 py-1 bg-zinc-900 border border-white/10 rounded text-[8px] font-bold text-zinc-500 hover:text-white transition-colors">MAX</button>
                              </div>
                           </div>

                           <div className="p-4 rounded-xl bg-zinc-900/50 border border-white/5 space-y-3">
                              <div className="flex justify-between text-[10px]">
                                 <span className="text-zinc-500">Est. Monthly Yield</span>
                           ...
                           <button 
                            ...
                            className="w-full py-4 bg-primary text-black rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-white active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-30 shadow-xl"
                           >
                   ...
                   <p className="text-[10px] text-zinc-600 text-center italic font-light">"Staking supports the protocol's liquidity and settlement efficiency."</p>
                </div>
              )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {result.technologies.map((t, i) => (
                    <div key={i} className="p-4 bg-white/5 rounded-xl border border-white/5 group hover:border-white/10 transition-colors">
                      <p className="text-[7px] md:text-[8px] font-bold text-zinc-600 uppercase tracking-widest mb-1">{t.category}</p>
                      <p className="text-xs font-medium text-zinc-300">{t.tech.join(', ')}</p>
                    </div>
                  ))}
                </div>
              )}
              {activeTab === 'BLUEPRINT' && (
                <div className="space-y-4">
                   <p className="text-xs md:text-sm text-zinc-500 leading-relaxed font-light">Detailed architectural schematics for smart contract governance and treasury management are available in the full executive report.</p>
                   <button className="flex items-center gap-2 text-[9px] font-bold text-primary uppercase tracking-widest hover:underline">
                      <FileText className="w-3 h-3" /> Download Full Blueprint
                   </button>
                </div>
              )}
           </div>
      ...
      <div className="fixed bottom-6 right-6 z-[150]">
        ...
        >
          ...
          <span className="absolute right-full mr-4 bg-black/80 backdrop-blur-md border border-white/10 px-3 py-1.5 rounded-lg text-[10px] font-bold text-primary uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none hidden md:block">
            Chat with Analyst
          </span>
        </button>
      </div>
           ...
           <div className="w-full max-w-lg h-[80vh] md:h-[600px] bg-surface border-t md:border border-white/10 rounded-t-[2rem] md:rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 duration-300">
              ...
              <form onSubmit={handleSendMessage} className="p-5 md:p-6 bg-black/40 border-t border-white/5 flex items-center gap-3 md:gap-4 shrink-0">
                <input 
                  ...
                  placeholder="Ask a technical or yield question..." 
